    ...
    for i in range(len(network)-1,-1,-1):
        ...
        if i!=len(network)-1:
            ...
            for j in range(len(layer)):
               ...
               # print(i,j,neuron)
def update_weights(network,learning_rate,momentum,data):
    inp=data
        ...
        for neuron in layer:
            for j in range(len(neuron['weights'])):
                if j!=len(neuron['weights'])-1:
                    neuron['alpha'][j]=momentum*neuron['alpha'][j]+learning_rate*neuron['delta']*inp[j]
                    neuron['weights'][j]+=neuron['alpha'][j]
                else:
                    neuron['alpha'][j] = momentum * neuron['alpha'][j] + learning_rate * neuron['delta']
                    neuron['weights'][j] += neuron['alpha'][j]
        ninp=[neuron['output'] for neuron in network[i]]
        inp=copy.deepcopy(ninp)

def MLP_classifier(n_inputs,n_outputs,n_hidden,n_epochs,learning_rate,momentum,activation_function,X,y):
    network=[]
    for i in range(len(n_hidden)):
        if i==0:
            hidden=[{'weights':[random.random() for j in range(n_inputs+1)],'alpha': [0 for j in range(n_inputs + 1)]}for k in range(n_hidden[i])]
            network.append(hidden)
        else:
            #hidden = [{'weights': [random.random() for j in range(n_hidden[i-1] + 1)]} for k in range(n_hidden[i])]
            hidden = [{'weights': [random.random() for j in range(n_hidden[i-1] + 1)],'alpha': [0 for j in range(n_hidden[i-1] + 1)]} for k in range(n_hidden[i])]
            network.append(hidden)
    #output = [{'weights': [random.random() for j in range(n_hidden[len(n_hidden)-1] + 1)]} for k in range(n_outputs)]
    output = [{'weights': [random.random() for j in range(n_hidden[i - 1] + 1)],'alpha': [0 for j in range(n_hidden[i - 1] + 1)]} for k in range(n_outputs)]
    ...
    for i in range(n_epochs):
        for j in range(len(X)):
            ...
            update_weights(network,learning_rate,momentum,data)
    return network
...
if __name__ == '__main__':
    X,y=make_moons(n_samples=500,noise=0.1,random_state=4)
    '''iris = load_iris()
    X = iris.data
    y = iris.target'''
    #print(X[0:5])
    ...
    mlp = MLP_classifier(len(X_train[0]), len(set([i for i in y_train])), [12], 100, 0.001,0.9,'erlu', X_train, y_train)
